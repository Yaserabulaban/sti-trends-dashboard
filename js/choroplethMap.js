...
export function renderChoroplethMap(rows, world, state) {
  ...
  const color = d3.scaleSequentialSqrt(d3.interpolateYlOrRd).domain([0, maxValue]);
  ...
  g.append("g")
    ...
    .attr("fill", (feature) => {
      ...
      return item ? color(item.value) : "#e2e8f0";
    })
    .attr("stroke", (feature) => {
      const item = byCode.get(countryKey(feature));
      return item?.row.countryName === state.selectedCountry ? "#111827" : "#ffffff";
    })
  ...
  svg.call(zoom);
  resetZoom = () => svg.transition().duration(450).call(zoom.transform, d3.zoomIdentity);
}
...
function drawLegend(svg, color, width, height, maxValue) {
  ...
  svg.append("text").attr("x", legendX).attr("y", legendY - 6).attr("class", "chart-title-note").text("Low");
  svg.append("text").attr("x", legendX + legendWidth).attr("y", legendY - 6).attr("text-anchor", "end").attr("class", "chart-title-note").text(formatValue(maxValue));
}
