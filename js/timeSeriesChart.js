...
export function renderTimeSeriesChart(rows, state) {
  ...
  const { svg, innerWidth, innerHeight, margin } = chartSvg("#time-series-chart", 390);
  ...
  g.selectAll(".series-point")
    .data(nested.flatMap((series) => series.values.map((point) => ({ ...point, key: series.key }))))
    .join("circle")
    .attr("r", 3)
    .attr("cx", (d) => x(d.year))
    .attr("cy", (d) => y(d.value))
  ...
  drawLegend(svg, nested.map((d) => d.key), color, innerWidth + margin.left, margin.top + 4);
...
function drawLegend(svg, keys, color, x, y) {
  const legend = svg.append("g").attr("class", "legend").attr("transform", `translate(${x - 150},${y})`);
