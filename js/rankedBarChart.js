...
import { chartColors, chartSvg, emptyState, filteredRows, formatValue, getActiveDisease, getRowValue, getValueLabel, getYearLabel, tooltipRows } from "./utils.js";
...
export function renderRankedBarChart(rows, state) {
  ...
  if (!ranked.length) {
    emptyState(
      "#ranked-bar-chart",
      `No ranked country data for ${getValueLabel(state)} in ${yearLabel}.`,
